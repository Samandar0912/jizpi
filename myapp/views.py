...
def kafedra(request):
    kafedra = ArticleKafedra.objects.all()
    return render(request, 'rektorat/kafedra/kafedra.html', {'kafedra': kafedra})   

def kafedra_detail(request, pk):
    kaf = ArticleKafedra.objects.all()
    kafedra = get_object_or_404(ArticleKafedra, pk=pk)
    context = {
        'kafedra': kafedra,
        'kaf':kaf,
    }
    return render(request, 'rektorat/kafedra/kafedra_detail.html', context)
